...
const FormSupervisor = () => {
  ...
  useEffect(() => {
    const peticion = async () => {
      const res = fetch(`http://localhost:3000/api/supervisor`);
      const supervisores = (await res).json();

      const { body } = await supervisores;
  ...
  const handleSubmit = async (event) => {
    ...
    const res = await fetch(`http://localhost:3000/api/supervisor`, {
      method: "POST",
      headers: {
